...
fn convert_fracts(l: Vec<(i64, i64)>) -> Vec<(i64, i64)> {
    ...
    let primes = generate_primes(l.iter().map(|p| p.1).max().unwrap_or(0).to_owned());
    let decompose = l
        .iter()
        .map(|&d| (prime_decomposition(d.0, &primes),prime_decomposition(d.1, &primes)) )
        .collect::<Vec<(Vec<(i64, i64)>, Vec<(i64, i64)>)>>();
    let denoms = decompose.iter().map(|(n, d)| {
        d.iter().map(|&el| {
            match n.iter().find(|&&num| num.0 == el.0) {
                Some(&found) => { (el.0, i64::max(el.1 - found.1, 0))},
                _ => { el }
            }
        }).collect::<Vec<(i64, i64)>>()
            .iter().filter(|el| el.1 != 0)
            .map(|e| e.to_owned())
            .collect::<Vec<(i64, i64)>>()
...
fn main() {
    // let v: Vec<(i64, i64)> = vec![(1, 0), (2, 1), (3, 1), (4, 0)];
    // let vv = v.iter().filter(|el| el.1 != 0).map(|e| e.to_owned()).collect::<Vec<(i64, i64)>>();
    // println!("{:?}", vv);
