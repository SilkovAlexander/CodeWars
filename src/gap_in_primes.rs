...
fn gap(g: i32, m: u64, n: u64) -> Option<(u64, u64)> {
    ...
    let mut int = 3;
    while int < ((n as f64).sqrt().floor() as u64) {
        ...
        if !break_flag {
            let last = primes.last().unwrap().to_owned();
            if (last > m) && (int - last == g as u64) {
                return Some((last, int));
            }
            primes.push(int);
        }
        int += 2;
    }
    int = max(int, if m % 2 == 1 { m } else { m + 1 });
    while int < n {
        let mut break_flag = false;
        for prime in &primes {
            if int % prime == 0 {
