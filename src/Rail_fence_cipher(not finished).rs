...
fn decode_rail_fence_cipher(text: &str, num_rails: usize) -> String {
    ...
    match num_rails {
        ...
        _ => {
            let chunk_len = 2 * num_rails - 2;
            let (chunk_cnt, last_size) = ((text.len() + chunk_len - 1) / chunk_len, text.len() % chunk_len);

            let count_layer_cnt = |layer:usize| {
                if layer == 0 {
                    chunk_cnt
                } else if layer == chunk_cnt - 1 {
                    if last_size > (num_rails - 1) {
                        chunk_cnt
                    } else {
                        chunk_cnt - 1
                    }
            ...
            let mut chunks = vec![vec!['\0'; chunk_len]; chunk_cnt];
            let mut index = 0;
            for j in 0..chunk_len {
                for i in 0..chunk_cnt {
                    // chunks[i][j] = text[index];
                    index += 1;
                }
            }
            "".to_owned()
